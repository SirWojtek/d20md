...
export class SavePanelComponent implements OnInit {
  ...
  constructor(private monstersService: MonstersService) {
    this.monstersService.getHasChangedObservable()
    .subscribe(() => this.ngOnInit());
  }

  ngOnInit() {
    this.monstersService.getMonster(this.monsterId, 'basic')
    .subscribe(monster => this.save = monster.Save);
  }

  onSaveChange(newVal: Save) {
    this.monstersService.updateMonster({ id: this.monsterId, Save: toJson(newVal) })
